...
export default function Sidebar() {
  ...
  return (
    <div
      className={`h-screen bg-customGrayBackground p-7 border-r-2 z-30 border-white-100 transition-all duration-300 ${
        isHovered ? "w-48" : "w-16"
      }`}
        ...
        {navigation.map((item, index) => (
          <li key={index}>
            <div className="inline-flex">
                ...
                <div className={`h-5 m-3.5 ml-0 transition-all duration-200  ${isHovered?"":"opacity-0 "}`}>{item.name}</div>
            </div>
          </li>
        ))}
      </ul>
